...
venues = [{ address: "123 Main Street", city: "Toronto", wheelchair_accessible: true, capacity: 100 },
...
#input is the venues hash
#define a possible venue array which takes hashes in as elements
possible_venues = []
#go inside the venues array
#loop through each individual venue
venues.each do |individual_venue|
#during looping check if the value for city is equal to toronto
#during looping check if the value for capacity is 150 or more
if individual_venue[:city] == "Toronto" && individual_venue[:capacity] >= 150
# if both conditions are true send that entire hash to the possible venues array
    possible_venues.push(individual_venue)
#end the if condition
end 
#end the loop
end
#display the possible venues array
